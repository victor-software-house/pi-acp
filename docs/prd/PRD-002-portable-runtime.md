...
- **`SshBackend`** — Bun Shell `$` invokes `perl -e 'alarm shift @ARGV; exec @ARGV or die "exec: $!\n"' <sec> ssh ...`. perl ships on macOS (`/usr/bin/perl`) and every Linux distribution we target, so the shell-level wall-clock timeout needs no extra tooling. `alarm` raises SIGALRM after N seconds → SshBackend reads exit 142 as "ssh timeout". ssh's own `-o BatchMode=yes -o ConnectTimeout=N -o ServerAliveInterval=2 -o ServerAliveCountMax=N` self-terminate on a stalled remote long before perl's alarm fires; perl is the belt-and-suspenders. macOS does not ship coreutils' `timeout(1)`, and `ShellPromise` has no `.timeout()` primitive (verified at runtime against bun 1.3.14 — only `cwd/env/quiet/nothrow/throws/text/json/lines/arrayBuffer/bytes/blob/run/then`). Honors operator's `~/.ssh/config` (ProxyJump, ControlMaster, agent forwarding). Phase 6 scope: AGENTS files via explicit `paths.agentsFiles` list only; skills / prompts / extensions over SSH stay deferred and surface one diagnostic per declared kind. Tool-selection rule: Bun Shell `$` for everything reachable from TypeScript (auto-escaped interpolation); inline perl for the gaps `$` itself cannot fill (timeouts, signal-handling) since perl ships everywhere; uv-shebanged Python under `scripts/` only when a helper grows beyond a one-liner and needs richer argparse or stdlib modules. Reserve `Bun.spawn` for cases needing IPC, AbortSignal-driven cancel, FileSink incremental stdin, or `maxBuffer`. Never `child_process.spawn`.
...
| FR-2 (`SshBackend`) | `bun-shell` | Bun Shell `$` runs `perl -e 'alarm shift; exec @ARGV or die'` inline; perl ships on macOS + Linux so no extra tool is needed for shell-level timeout. ssh's own `ServerAliveInterval`/`ConnectTimeout` handle stalled remotes. Reserve `Bun.spawn` for IPC / AbortSignal / FileSink stdin / maxBuffer. uv-shebanged Python (`uv-python-cli` skill) only when a helper outgrows a one-liner. Never `child_process.spawn`. |
