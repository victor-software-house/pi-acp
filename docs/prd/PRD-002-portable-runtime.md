...
6. **Phase 8** — FR-6 `read` delegation + `AcpFsBackend`. Split into 8a/8b. **Phase 8a (Shipped)** — FR-6 `read` tool ACP-FS delegation via pi's `ReadOperations` injection seam (no fully-custom-tool dance). Capability-gated on `clientCapabilities.fs.readTextFile`. Allowlist must include `"read"` so the override customTool registers and then shadows the builtin via `AgentSession`'s tool-definition `Map.set` path — verified against pi source (PRD wording was incorrect on that point). SessionId bound late via mutable ref because newSession/forkSession mint the id inside `createAgentSession`; tool isn't invoked until prompt-time so the late binding is safe. **Phase 8b (deferred)** — `AcpFsBackend` for AGENTS-file discovery has an architectural snag: sessionId isn't available at `buildResourceLoader` time. Options: late-bound reload, pre-mint sessionId outside pi, or drop the use case if SSH source covers it.
7. **Phase 9** — `import_resource` custom tool. *(Deferred — needs empirical verification of PRD-002 §12 Q1: does `resourceLoader.extendResources()` propagate mid-session, or only at the next `reload`? Pi internals show `extendResources` mutates the loader's path maps but session-side skill/prompt registries are rebuilt only at session-start or explicit reload. Tool would need to call `piSession.reloadResources()` (verify the surface) or stage imports for the next prompt cycle. Decision deferred to follow-up.)*
8. **Phase 10** — Cwd-independence modes (`overlay`, `none`). *(Shipped — `src/resources/modes.ts` with `resolveMode` returns `{ mode, cwd, cleanup, ephemeral }`. `none` mints `mkdtempSync(...pi-acp-session-*)` under OS tmpdir; cleanup wired through `PiAcpSession.dispose()` via new `cleanups[]` constructor opt. `local` and `overlay` documented as functionally identical in the current substrate — both pass `params.cwd` through unchanged; the `overlay` keyword is retained for operator clarity. 6 unit tests cover passthrough, ephemeral tmpdir mint + cleanup, cleanup idempotence, cleanup-after-external-removal safety, distinct tmpdirs per session.)*
9. **Phase 11** — Diagnostics surfacing + final docs. *(Shipped — `src/resources/diagnostics.ts` ships `buildDiagnosticsReport` formatting a two-block summary (`resources active` + `resource failures`). Opt-in via manifest `diagnostics: true`; emitted as a leading `agent_message_chunk` on the first prompt and cleared after. Source-level diagnostics + manifest-level diagnostics (parse failures, unsupported root kinds) flow through the same report. 6 unit tests cover empty input, sources-only block, source-level failures, manifest-level failures, combined diagnostics, stat counts.)*

**Substrate status (post-Phase 11):** Phases 0/4/5/6/7/8a/10/11 shipped; Phases 8b (AcpFsBackend) and 9 (import_resource) deferred to follow-up with explicit architectural notes. The runtime tag corresponding to "v0.6 substrate complete" in the pi-acp package version stream is `v0.13.0` (pi-acp uses semantic-release auto-bumping; the package version is decoupled from the PRD's logical "v0.6" milestone naming).
