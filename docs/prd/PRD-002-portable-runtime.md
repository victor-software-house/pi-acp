...
- **`SshBackend`** — uses the system `ssh` command via `Bun.spawn(["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=N", target, "--", "cat", path])` array-form. Argv interpolation is injection-safe (no shell expansion). Per-operation 5s default timeout via Bun.spawn's native `timeout` + `killSignal: "SIGKILL"` — Bun Shell `$` lacks a timeout primitive, so the `bun-shell` skill's "When you still need Bun.spawn" matrix selects `Bun.spawn`. Honors user's `~/.ssh/config` (ProxyJump, ControlMaster, agent forwarding). No in-process SFTP dependency. Phase 6 scope: AGENTS files via explicit `paths.agentsFiles` list only; skills / prompts / extensions over SSH stay deferred and surface one diagnostic per declared kind. Helper scripts that do not justify a TypeScript module (e.g., fixture seeders) live as uv-shebanged Python files under `scripts/` (`#!/usr/bin/env -S uv run --script` with PEP 723 inline metadata) and are invoked the same way from `$` or `Bun.spawn`. Tool-selection rule: Bun Shell `$` for throwaway shell pipelines reachable from TypeScript; Bun.spawn for anything that needs `timeout` / `killSignal` / `signal` / `maxBuffer` / IPC; uv-shebanged Python only when the Python ecosystem is genuinely better (rich CLI parsing via typer, scientific libs).
...
| FR-2 (`SshBackend`) | `bun-shell`, `uv-python-cli` | Bun.spawn array-form for the runtime SSH call (needs native `timeout` + `killSignal`); Bun Shell `$` for throwaway shell pipelines; uv-shebanged Python for non-runtime helper scripts. Never `child_process.spawn`. |
